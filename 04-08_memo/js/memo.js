...
function allClearLocalStorage() {
    ...
    allClear.addEventListener("click",
        function (e) {
            e.preventDefault();
            let w_confirm = window.confirm("LocalStorageのデータを全て削除(delete)します。\nよろしいですか？");
            if (w_confirm === true) {
                localStorage.clear();
                viewStorage();
                let w_msg = "LocalStorageのデータを全て削除(delete)しました。";
                window.alert(w_msg);
                document.getElementById("textKey").value = "";
                document.getElementById("textMemo").value = "";
            }
...
function selectCheckBox(mode) {
    ...
    for (let i = 0; i < chkbox1.length; i++) {
        if (chkbox1[i].checked) {
            if (w_cnt === 0) {
                ...
                w_texMemo = table1.rows[i + 1].cells[2].firstChild.data;
                //return w_sel = "1";
            }
            w_cnt++
        }
    ...
    document.getElementById("textMemo").value = w_texMemo;
    if (mode === "select") {
        if (w_cnt === 1) {
            return w_cnt;
        } else {
